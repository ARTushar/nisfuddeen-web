...
export default Joi.object({
    ...
    aboveKnee: Joi.boolean()
      ...
      .when('queryType', {is: '4m', then: Joi.required()}),
    outfit: Joi.array()
      .when('queryType', {is: '3m', then: Joi.array().items(Joi.string().valid(...getKeys(BoyOutfit)))
            .unique().max(getKeys(BoyOutfit).length).required()})
      .when('queryType', {is: '3f', then: Joi.array().items(Joi.string().valid(...getKeys(GirlOutfit)))
            .unique().max(getKeys(GirlOutfit).length).required()})
      .when('queryType', {is: '4m', then: Joi.array().items(Joi.string().valid(...getKeys(BoyOutfit)))
            .unique().max(getKeys(BoyOutfit).length).required()})
      .when('queryType', {is: '4f', then: Joi.array().items(Joi.string().valid(...getKeys(GirlOutfit)))
            .unique().max(getKeys(GirlOutfit).length).required()}),
