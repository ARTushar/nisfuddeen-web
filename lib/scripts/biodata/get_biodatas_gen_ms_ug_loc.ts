...
import { getBiodatasByGnMsLoc, getBiodatasByGnMsUgLoc } from '../../dataAccessLayer/entities/biodata/getBiodata';
...
(async() => {
    if (argv.gender && argv.ms && argv.ug) {
        ...
        try {
            ...
            const biodatas = await getBiodatasByGnMsUgLoc({
                enabled: enabled,
                verified: verified,
                gender: argv.gender,
                maritalStatus: argv.ms,
                ugradInstitute: argv.ug,
